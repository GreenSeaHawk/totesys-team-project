...
from src.transform_handler import (
    get_data_from_files,
    add_to_s3_file_json,
...
def test_lambda_handler_success(
    ...
    mock_add_to_s3_file_parquet.return_value = (
        None  # for simulating successful upload
    )
...
def test_lambda_handler_missing_last_key(
    mock_list_all_filenames_in_s3, mock_event, mock_context
):
    ...
    assert (
        "An unexpected Error occured. Error retrieving last_run.json"
        in result["body"]
    )
