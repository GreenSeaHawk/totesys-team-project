...
def test_transform_handler_calls_utils_when_only_currency_data(
    ...
    def side_effect_list_files(
            ...
            last_run_timestamp,
            prefix):
        if prefix == 'currency' and Bucket == source_bucket and last_run_timestamp == 'test-last-timestamp':
    ...
    mock_get_data_from_files.assert_called_once_with(
        Bucket=source_bucket,
        list_of_files='currency files')
    mock_transform_to_dim_currency.assert_called_once_with(currency_data='currency_data')
...
def test_transform_handler_works_when_all_utils_are_called(
    ...
    mock_transform_to_fact_purchase_order.return_value = 'transformed_purchase_order'
    ...
    mock_transform_to_dim_payment_type.return_value = 'transformed_payment_type'
    ...
    mock_transform_to_dim_currency.return_value = 'transformed_currency'
    mock_transform_to_dim_counterparty.return_value = 'transformed_counterparty'
    ...
    mock_filter_latest_data.assert_any_call(
        data='department all data',
        key='department_id'
    )

    mock_filter_latest_data.assert_any_call(
        data='address all data',
        key='address_id'
    )
    ...
    mock_transform_to_dim_staff.assert_called_once_with(
        ...
        department_data='department filtered data')
