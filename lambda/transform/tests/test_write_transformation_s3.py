...
def test_write_transformed_data_to_s3_success(mock_add_to_s3_file_parquet, caplog):
    ...
    # Assertions
    assert f'Writing transforming data for table {table_name} to S3' in caplog.text
    assert f'Transformation completed sucessfully for table {table_name}.' in caplog.text

@patch("src.write_transfomed_data_to_s3.add_to_s3_file_parquet")
def test_write_transformed_data_to_s3_logs_failure(mock_add_to_s3_file_parquet, caplog):
    ...
    # Assertions
    assert f'Writing transforming data for table {table_name} to S3' in caplog.text
    assert f'Failed to write transformed data for table {table_name} to s3: Mock S3 Failure.' in caplog.text