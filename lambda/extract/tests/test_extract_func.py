...
def test_extract_data_from_table():
    ...
    query = (
        ...
        "WHERE created_at > '1900-01-01' OR last_updated > '1900-01-01'"
    )
    pd.read_sql.assert_called_once_with(
        query,
        mock_connection,
    )
...
class TestSerialiseData:
    ...
    def test_serialise_data_invalid_format(self):
        with pytest.raises(
            ValueError,
            match="unsupported format. It can be json, csv, or parquet",
        ):
