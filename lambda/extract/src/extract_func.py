...
def extract_table_data(db_conn, table_name, last_ran):
    """extracting data from a specified table and returning
    it as a dataframe"""
    query = (
        f"SELECT * FROM {table_name} "
        f"WHERE created_at > '{last_ran}' OR last_updated > '{last_ran}'"
    )
...
def serialise_data(data, format="json"):
    ...
    if format == "json":
        ...
        raise ValueError("unsupported format. It can be json, csv, or parquet")
